...
import {
    Truck, Plus, Trash2, Calculator, Save,
    Search, ArrowRight, X, UserCircle, Package,
    TrendingUp, Clock, Loader2, Hammer, Wallet
} from "lucide-react";
...
export default function NewPurchaseBillPage() {
    ...
    useEffect(() => {
        ...
        const fetchFarmersList = async () => {
            ...
            try {
                const res = await fetch(`/api/farmers?search=${encodeURIComponent(debouncedFarmerSearch)}&page=${farmerPage}&limit=20`, { signal: controller.signal });
                const result = await res.json();
                if (result.data) {
                    setFarmersList(prev => farmerPage === 1 ? result.data : [...prev, ...result.data]);
                    setHasMoreFarmers(result.pagination.page < result.pagination.totalPages);
                }
            } finally {
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
            ...
            {/* ── Main Grid ── */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "1.5rem" }} className="purchase-bill-grid">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                    ...
                    <div className="premium-card" style={{ padding: "1.75rem" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "1.25rem" }}>
                            ...
                            <p style={{ margin: 0, fontSize: "10px", fontWeight: 900, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.2em" }}>{t("bills.purchase.selectFarmer")}</p>
                        </div>

                        {selectedFarmer ? (
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1rem 1.25rem", backgroundColor: "rgba(21,128,61,0.06)", border: "1.5px solid rgba(21,128,61,0.15)", borderRadius: "16px" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                                    <div style={{ width: "44px", height: "44px", backgroundColor: "#15803d", color: "#fff", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: "18px" }}>
                                        {selectedFarmer.name.charAt(0)}
                                    </div>
                                    <div>
                                        <p style={{ margin: 0, fontWeight: 900, fontSize: "15px", color: "#14532d" }}>{selectedFarmer.name}</p>
                                        <p style={{ margin: "2px 0 0", fontSize: "12px", fontWeight: 700, color: "#15803d", opacity: 0.7 }}>{selectedFarmer.mobile}</p>
                                    </div>
                                </div>
                                <button onClick={() => setSelectedFarmer(null)} style={{ width: "36px", height: "36px", borderRadius: "50%", border: "none", backgroundColor: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#94a3b8", boxShadow: "0 1px 4px rgba(0,0,0,0.08)" }}
                            ...
                            <div style={{ position: "relative" }}>
                                ...
                                <input type="text" placeholder={t("bills.purchase.farmerPlaceholder")} value={farmerSearch}
                                    ...
                                    onBlurCapture={e => { setTimeout(() => setIsFarmerDropdownOpen(false), 150); e.currentTarget.style.borderColor = "#e2e8f0"; e.currentTarget.style.backgroundColor = "#f1f5f9"; }} />
                                {isFarmerDropdownOpen && (debouncedFarmerSearch.length > 0 || farmersList.length > 0) && (
                                    <div
                                        ...
                                        onScroll={e => {
                                            ...
                                            if (scrollHeight - scrollTop <= clientHeight + 50 && hasMoreFarmers && !isFarmerLoading) {
                                                setFarmerPage(p => p + 1);
                                            }
                                        }}
                                    >
                                        {farmersList.map(f => (
                                            <button key={f.id} onMouseDown={() => { setSelectedFarmer(f); setFarmerSearch(""); setIsFarmerDropdownOpen(false); }}
                                                ...
                                                <div style={{ textAlign: "left" }}>
                                                    <p style={{ margin: 0, fontWeight: 800, color: "#1e293b", fontSize: "14px" }}>{f.name}</p>
                                                    <p style={{ margin: "2px 0 0", fontSize: "12px", color: "#94a3b8", fontWeight: 600 }}>{f.mobile}</p>
                                                </div>
                                        ...
                                        {!isFarmerLoading && farmersList.length === 0 && debouncedFarmerSearch.length > 0 && (
                                            <p style={{ padding: "12px", margin: 0, textAlign: "center", fontSize: "12px", color: "#94a3b8", fontWeight: 700 }}>No farmers found</p>
                                        )}
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="premium-card" style={{ overflow: "hidden" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem", padding: "1.5rem 1.75rem", borderBottom: "1px solid var(--border-main)", background: "linear-gradient(to right, rgba(255,255,255,1), rgba(248,250,252,1))" }}>
                            ...
                            <div style={{ position: "relative", minWidth: "220px" }}>
                                ...
                                <input type="text" placeholder={t("bills.purchase.addItem")} value={itemSearch}
                                    ...
                                    onFocusCapture={e => { e.currentTarget.style.borderColor = "#15803d"; e.currentTarget.style.backgroundColor = "#fff"; }}
                                    onBlurCapture={e => { setTimeout(() => setIsItemDropdownOpen(false), 150); e.currentTarget.style.borderColor = "#e2e8f0"; e.currentTarget.style.backgroundColor = "#f1f5f9"; }} />
                        ...
                        {lines.length === 0 ? (
                            ...
                            <div>
                                {/* Desktop Table */}
                                <table style={{ width: "100%", borderCollapse: "collapse" }} className="purchase-items-table">
                                    <thead>
                                        ...
                                        {lines.map((line, idx) => (
                                            <tr key={idx} style={{ borderBottom: "1px solid #f1f5f9" }}>
                                                <td style={{ padding: "16px 20px" }}>
                                                    ...
                                                    <span style={{ fontSize: "10px", fontWeight: 800, color: "#15803d", backgroundColor: "rgba(21,128,61,0.08)", padding: "2px 8px", borderRadius: "6px", textTransform: "uppercase" }}>
                                                ...
                                                <td style={{ padding: "16px 20px", textAlign: "right", fontWeight: 900, fontSize: "15px", color: "#0f172a" }}>{fmt(line.total)}</td>
                                                <td style={{ padding: "16px" }}>
                    ...
                    <div className="premium-card" style={{ padding: "1.75rem" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "1.5rem" }}>
                            <div style={{ width: "36px", height: "36px", backgroundColor: "rgba(239,68,68,0.1)", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", color: "#ef4444" }}>
                                <Wallet size={18} strokeWidth={2} />
                            </div>
                            <p style={{ margin: 0, fontSize: "10px", fontWeight: 900, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.2em" }}>Extra Charges & Deductions</p>
                        </div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1.5rem" }}>
                            ...
                            ].map(({ label, value, set, color, bg, icon: Icon }) => (
                                <div key={label}>
                                    <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
                                        <div style={{ width: "24px", height: "24px", backgroundColor: bg, borderRadius: "6px", display: "flex", alignItems: "center", justifyContent: "center", color }}>
                                            <Icon size={12} strokeWidth={2.5} />
                                        </div>
                                        <p style={{ margin: 0, fontSize: "10px", fontWeight: 900, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.1em" }}>{label}</p>
                                    </div>
                                    <div style={{ position: "relative" }}>
                                        <span style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", fontSize: "12px", fontWeight: 800, color: "#94a3b8" }}>₹</span>
                                        <input type="text" inputMode="decimal" value={value}
                                            ...
                                            style={{ width: "100%", boxSizing: "border-box", padding: "12px 12px 12px 28px", backgroundColor: "#f1f5f9", border: "1.5px solid #e2e8f0", borderRadius: "12px", fontWeight: 800, fontSize: "15px", color: "var(--text-main)", outline: "none", transition: "all 0.2s" }}
                    ...
                    <div className="premium-card" style={{ padding: "1.75rem", backgroundColor: "#0f172a", border: "none", color: "#fff" }}>
                        ...
                        <div style={{ marginTop: "1.5rem", padding: "1.25rem", backgroundColor: "rgba(255,255,255,0.04)", borderRadius: "14px", border: "1px solid rgba(255,255,255,0.06)" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "0.5rem" }}>
                                ...
                                <span style={{ fontSize: "10px", fontWeight: 900, color: "#34d399", textTransform: "uppercase", letterSpacing: "0.15em" }}>{t("bills.purchase.netTotal")}</span>
                            </div>
