...
export default function NewPurchaseBillPage() {
    ...
    useEffect(() => {
        if (farmerSearch.length > 1) {
            fetch(`/api/farmers?search=${encodeURIComponent(farmerSearch)}`)
                .then(r => r.ok ? r.json() : []).then(d => setFarmersList(d || [])).catch(() => { });
        } else setFarmersList([]);
    }, [farmerSearch]);
    ...
    useEffect(() => {
        if (itemSearch.length > 0) {
            fetch(`/api/items?search=${encodeURIComponent(itemSearch)}`)
                .then(r => r.ok ? r.json() : []).then(d => setItemsList(d || [])).catch(() => { });
        } else setItemsList([]);
    }, [itemSearch]);
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
                ...
                <Link href="/dashboard/bills" style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "10px 20px", backgroundColor: "#f1f5f9", color: "#64748b", borderRadius: "12px", fontWeight: 800, fontSize: "14px", textDecoration: "none", transition: "all 0.2s" }}
